...
const convertPercentageStop = (node) => call('percentage', [
    numericLiteral(Number(node.value)),
]);

const convertStop = (node) => {
    if (node.type === 'TypeSelector')
        return createStringLiteral(node.name);
    
    return convertPercentageStop(node);
};

const getKeyframesName = (node) => {
    const [nameNode] = node.prelude.children.toArray();
...
export function convertKeyframes(node, comments) {
    ...
    return expressionStatement(call('keyframes', [
